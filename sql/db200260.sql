...
( 
	...
	CONSTRAINT [DF_NULL_1874707671]
		 DEFAULT  NULL
	CONSTRAINT [CK_GreaterThanOrEqualToZero_1941437412]
		CHECK  ( Price >= 0 ),
...
CREATE UNIQUE CLUSTERED INDEX [XPKDrive] ON [Drive]
( 
	[IdUser]              ASC,
	[IdPkg]               ASC
)
...
CREATE NONCLUSTERED INDEX [XIF1Drive] ON [Drive]
( 
	[IdUser]              ASC
)
go

CREATE NONCLUSTERED INDEX [XIF2Drive] ON [Drive]
( 
	[IdPkg]               ASC
)
go
...
ALTER TABLE [Drive]
	ADD CONSTRAINT [R_29] FOREIGN KEY ([IdPkg]) REFERENCES [Package]([IdPkg])
		ON DELETE NO ACTION
		ON UPDATE NO ACTION
go
...
ALTER TABLE [RidePackages]
	ADD CONSTRAINT [R_25] FOREIGN KEY ([IdUser],[IdPkg]) REFERENCES [Drive]([IdUser],[IdPkg])
		ON DELETE CASCADE
		ON UPDATE CASCADE
go
